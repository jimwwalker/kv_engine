...
Options:
  ...
  --continue-time-limit          How long a continue can execute for (ms),
                                 default is no limit.
  --vbucket                      Scan only this vbucket
  --help                         This help text
)";
...
public:
    ...
    void enterMessagePump(const nlohmann::json& scanConfig) {
        ...
        for (auto vb : vbuckets) {
            totalScans++;
            BinprotGenericCommand rangeScanCreate(
                    cb::mcbp::ClientOpcode::RangeScanCreate);
            rangeScanCreate.setVBucket(Vbid(vb));
            rangeScanCreate.setValue(scanConfig.dump());
            rangeScanCreate.setDatatype(cb::mcbp::Datatype::JSON);
            auto id = ++opaque;
...
protected:
    ...
    void handleCreateResponse(const cb::mcbp::Response& response) {
        ...
        auto vb = creates[response.getOpaque()];
    ...
    void continueNextScan() {
        ...
        cb::mcbp::request::RangeScanContinuePayload extras(
                scans[vb], continueItemLimit, continueTimeLimit.count());
        BinprotGenericCommand rangeScanContinue(
                cb::mcbp::ClientOpcode::RangeScanContinue);
        rangeScanContinue.setExtras(extras.getBuffer());
        rangeScanContinue.setVBucket(Vbid(vb));
    ...
    bool handleContinueResponse(const cb::mcbp::Response& response) {
        ...
        // This is the final 24b of the continue stream, and it tells us to try
        // for more data
        if (response.getStatus() == cb::mcbp::Status::RangeScanMore) {
            auto vb = opaques[response.getOpaque()];
            opaques.erase(response.getOpaque());
            ready.push(vb);
            return true;
        } else if (response.getBodylen() == 0) {
            // end of scan
            auto vb = opaques[response.getOpaque()];
            opaques.erase(response.getOpaque());
            scans.erase(vb);
        ...
        // Don't yet try for another scan, we must keep reading this continue
        return false;
    }
...
int main(int argc, char** argv) {
    ...
    if (false /*csv*/) {
        std::cout << duration.count() << ';' << total_bytes << ';' << records
                  << std::endl;
    } else {
