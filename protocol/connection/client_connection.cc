...
void MemcachedConnection::dcpStreamRequestResponse(
        uint32_t opaque, std::vector<std::pair<uint64_t, uint64_t>> failovers) {
...
size_t MemcachedConnection::dcpMutation(const Document& doc,
                                        ...
                                        uint8_t nru) {
    // No reply expected
    return sendCommand(BinprotDcpMutationCommand{std::move(doc.info.id),
                                                 doc.value,
                                                 opaque,
...
size_t MemcachedConnection::dcpDeletionV2(const Document& doc,
                                          ...
                                          uint32_t deleteTime) {
    // No reply expected
    return sendCommand(BinprotDcpDeletionV2Command{std::move(doc.info.id),
                                                   doc.value,
                                                   opaque,
                                                   uint8_t(doc.info.datatype),
                                                   doc.info.cas,
...
void MemcachedConnection::recvDcpBufferAck(uint32_t expected) {
    ...
    if (request->getClientOpcode() !=
        ...
        throw std::logic_error("recvDcpBufferAck not a buffer ack opcode");
    }
    auto* dcpBufferAck =
    ...
    if (dcpBufferAck->getBufferBytes() != expected) {
        throw std::logic_error("Unexpected buffer bytes:" +
