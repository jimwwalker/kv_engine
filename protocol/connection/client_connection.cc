...
public:
    ...
    size_t getBytesWritten() const {
        // When checksumming use the passed through count, which is the number
        // of bytes - the checksums, else the processed value is correct
    ...
    /// The checksum sink (if checksumming is enabled)
...
uint64_t MemcachedConnection::getFileFragment(
    ...
    // Return the bytes written to the given sink
