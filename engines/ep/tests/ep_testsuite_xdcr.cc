...
static enum test_result test_get_meta_with_extras(EngineIface* h) {
    ...
    ItemIface* i = nullptr;
    checkeq(cb::engine_errc::success,
            store(h, nullptr, StoreSemantics::Set, key1, "somevalue", &i),
            "Failed set.");
    ...
    check_expression(get_meta(h, key1, errorMetaPair), "Expected to get meta");
    ...
    if (isWarmupEnabled(h)) {
        ...
        check_expression(get_meta(h, key1, errorMetaPair),
                         "Expected to get meta");
        verifyMetaData(metadata1, errorMetaPair.second);
    }
...
static enum test_result test_get_meta_nonexistent(EngineIface* h) {
   ...
   checkeq(0, temp, "Expect zero getMeta ops");
    ...
    cb::EngineErrorMetadataPair errorMetaPair;
    check_expression(!get_meta(h, key, errorMetaPair),
...
static enum test_result test_get_meta_with_get(EngineIface* h) {
   ...
   checkeq(cb::engine_errc::success, del(h, key1, 0, Vbid(0)), "Delete failed");
   wait_for_flusher_to_settle(h);
   check_expression(get_meta(h, key1, errorMetaPair), "Expected to get meta");
   checkeq(DocumentState::Deleted,
           errorMetaPair.second.document_state,
   ...
   check_expression(!get_meta(h, key2, errorMetaPair),
                    "Expected get meta to return false");
   checkeq(cb::engine_errc::no_such_key,
           errorMetaPair.first,
           "Expected no_such_key");
...
static enum test_result test_get_meta_with_set(EngineIface* h) {
    ...
    checkeq(cb::engine_errc::no_such_key,
            ...
            "Expected no_such_key");
    checkeq(cb::engine_errc::success,
            store(h, nullptr, StoreSemantics::Set, key2, "someothervalue"),
            "Failed set.");
    // check the stat again
    checkeq(3,
            get_int_stat(h, "ep_num_ops_get_meta"),
...
static enum test_result test_get_meta_with_xattr(EngineIface* h) {
    ...
    // returned).
    check_expression(get_meta(h, key, errorMetaPair, cookie),
...
static enum test_result test_delete_with_meta(EngineIface* h) {
    ...
    checkeq(cb::engine_errc::success,
            store(h, nullptr, StoreSemantics::Set, key2, "somevalue2"),
            "Failed set.");
...
static enum test_result test_delete_with_meta_deleted(EngineIface* h) {
    ...
    // delete the key
    checkeq(cb::engine_errc::success, del(h, key, 0, Vbid(0)), "Delete failed");
    wait_for_flusher_to_settle(h);
    wait_for_stat_to_be(h, "curr_items", 0);
...
static enum test_result test_set_with_meta(EngineIface* h) {
    ...
    ItemMetaData itm_meta(0xdeadbeef, 10, 0xdeadbeef, makeExpiryTime(300));
    ...
    item_info info;
    check_expression(get_item_info(h, &info, key, Vbid(0)),
...
static enum test_result test_delete_with_meta_xattr(EngineIface* h) {
    ...
    cb::EngineErrorMetadataPair errorMetaPair;
    check_expression(get_meta(h, key1, errorMetaPair), "Failed get_meta(key1)");
...
static enum test_result test_exp_persisted_set_del(EngineIface* h) {
    ...
    check_expression(!get_meta(h, "key3", errorMetaPair),
...
static enum test_result test_temp_item_deletion(EngineIface* h) {
    ...
    check_expression(!get_meta(h, k1, errorMetaPair, cookie),
    ...
    ExecutorPool::get()->setNumReadersExactly(1);
    ...
    char const* k2 = "k2";
    check_expression(!get_meta(h, k2, errorMetaPair),
                     "Expected get meta to return false");
    checkeq(cb::engine_errc::no_such_key,
            errorMetaPair.first,
            "Expected no_such_key");
...
static enum test_result test_set_meta_conflict_resolution(EngineIface* h) {
    ...
    itemMeta.exptime = makeExpiryTime(10);
    checkeq(cb::engine_errc::success,
            set_with_meta(h, "key", {}, Vbid(0), &itemMeta, 0),
            "Expected item to be stored");
    checkeq(cb::mcbp::Status::Success, last_status.load(), "Expected success");
...
static enum test_result test_cas_regeneration(EngineIface* h) {
    ...
    check_expression(get_meta(h, "key", errorMetaPair), "Failed to get_meta");
...
static enum test_result test_mb_59744_set_meta_lww_after_locked(
        EngineIface* h) {
    ...
    checkeq(cb::engine_errc::success,
            set_with_meta(
                    h, key, {}, vb, &itemMeta, 0, FORCE_ACCEPT_WITH_META_OPS),
            "Expected item to be stored");
    ...
    checkeq(cb::engine_errc::success,
            locked.first,
            "Expected GetLocked to succeed");
    checkgt(locked.second->getCas(),
            itemMeta.cas,
            "Expected locked CAs to be greater than current CAS");

    // 3. Unlock the document. Before the fix for MB-59744, this would leave
    //    the document having the same increased locked_CAS.
    checkeq(cb::engine_errc::success,
            unl(h, cookie, key, vb, locked.second->getCas()),
    ...
    check_expression(get_meta(h, key, meta, cookie),
                     "Expected GetMeta to succeed");
    checkeq(itemMeta.cas, meta.second.cas, "Expected CAS to be updated");
    checkeq(uint64_t(itemMeta.revSeqno),
            meta.second.seqno,
