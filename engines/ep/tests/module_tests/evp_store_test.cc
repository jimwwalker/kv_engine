...
TEST_P(EPBucketTestNoRocksDb, ScheduleCompactionWithNewConfig) {
    ...
    EXPECT_EQ(ENGINE_EWOULDBLOCK,
              mockEPBucket->scheduleCompaction(vbid, c, nullptr, 0.0));
    ...
    EXPECT_EQ(ENGINE_EWOULDBLOCK,
              mockEPBucket->scheduleCompaction(vbid, nullptr, 0.0));
    EXPECT_EQ(c, task->getCurrentConfig());
...
TEST_P(EPBucketTestNoRocksDb, ScheduleCompactionReschedules) {
    ...
    EXPECT_EQ(ENGINE_EWOULDBLOCK,
              mockEPBucket->scheduleCompaction(vbid, config1, nullptr, 0.0));
    task = mockEPBucket->getCompactionTask(vbid);
    ASSERT_TRUE(task);
    ...
    EXPECT_EQ(ENGINE_EWOULDBLOCK,
              mockEPBucket->scheduleCompaction(vbid, config2, nullptr, 0.0));
    task = mockEPBucket->getCompactionTask(vbid);
    ASSERT_TRUE(task);
    ...
    task->setRunningCallback([this, &config3, mockEPBucket]() {
        EXPECT_EQ(
                ENGINE_EWOULDBLOCK,
                mockEPBucket->scheduleCompaction(vbid, config3, nullptr, 0.0));
    });
