...
public:
    ...
    auto doPrepareSnapshot(const std::filesystem::path& directory, Vbid vbid, bool generateChecksums) {
        MockCookie cookie;
        return kvstore->prepareSnapshot(cookie, directory, vbid, generateChecksums);
    }
...
TEST_P(SnapshotsTests, prepare) {
    auto rv = cache.prepare(
            vbid, [this](const auto& dir, auto vb) {
