...
TEST_P(CollectionsDcpPersistentOnly, ModifyCollection) {
    ...
    producer->stepAndExpect(*producers, ClientOpcode::DcpSystemEvent);
    EXPECT_EQ(producers->last_system_event, id::CreateCollection);
    EXPECT_EQ(producers->last_collection_id, fruit);
    ...
    EXPECT_EQ(producers->last_collection_id, vegetable);
...
TEST_P(CollectionsDcpPersistentOnly, ModifyCollectionTwoVbuckets) {
    ...
    auto producer2 = SingleThreadedKVBucketTest::createDcpProducer(
            cookieP, IncludeDeleteTime::Yes, true /*FlatBuffers*/);
    ...
    std::array<std::shared_ptr<MockDcpProducer>, 2> dcpProducers{producer,
                                                                 producer2};
