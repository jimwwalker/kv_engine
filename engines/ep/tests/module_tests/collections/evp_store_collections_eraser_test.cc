...
TEST_P(CollectionsEraserTest, erase_before_flush) {
    // add a collection
    vb->updateFromManifest({R"({"separator":":","uid":"0",)"
                            R"("collections":[{"name":"$default", "uid":"0"},)"
                            R"(               {"name":"fruit","uid":"1"},)"
    ...
    // Eraser will drop keys, but not be able to complete deletion yet as the
    // delete marker needed to trigger the final delete step hasn't been written
    runEraser();
