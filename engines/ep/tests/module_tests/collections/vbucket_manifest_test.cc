        ...
        for (const auto& qi : events) {
            ...
            if (qi->getOperation() == queue_op::system_event) {
                ...
                case SystemEvent::FlushCollection: {
                    EXPECT_FALSE(qi->isDeleted());
                    const auto& flush = Collections::VB::Manifest::
                            getFlushCollectionFlatbuffer(qi->getValueView());
                    folly::SharedMutex::ReadHolder rlh(vbR->getStateLock());
                    replica.wlock(rlh).replicaFlush(
                            *vbR,
                            Collections::ManifestUid{flush.uid()},
                            flush.collectionId(),
                            Collections::ManifestUid{flush.flushUid()},
                            qi->getBySeqno());
                    break;
                }
                default:
