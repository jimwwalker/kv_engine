...
public:
    ...
    std::optional<Manifest::CollectionCreation> public_applyCreatesOrFlushes(
            ::VBucket& vb, Manifest::ManifestChanges& changes) {
        folly::SharedMutex::ReadHolder rlh(vb.getStateLock());
        auto wHandle = wlock(rlh);
        return applyCreatesOrFlushes(
...
TEST_P(VBucketManifestTest, check_applyChanges) {
    ...
    auto value = manifest.getActiveManifest().public_applyCreatesOrFlushes(
    ...
    value = manifest.getActiveManifest().public_applyCreatesOrFlushes(
            manifest.getActiveVB(), changes);
