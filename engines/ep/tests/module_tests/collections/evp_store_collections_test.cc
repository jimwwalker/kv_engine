...
TEST_F(CollectionsExpiryLimitTest, set) {
...
TEST_F(CollectionsExpiryLimitTest, add) {
    auto func = [this](Vbid vb, DocKey k, std::string v) {
        auto item = make_item(vb, k, v);
        EXPECT_EQ(0, item.getExptime());
...
TEST_F(CollectionsExpiryLimitTest, replace) {
    auto func = [this](Vbid vb, DocKey k, std::string v) {
        auto item = make_item(vb, k, v);
        EXPECT_EQ(0, item.getExptime());
        EXPECT_EQ(ENGINE_SUCCESS, store->add(item, cookie));
...
TEST_F(CollectionsExpiryLimitTest, set_with_meta) {
    auto func = [this](Vbid vb, DocKey k, std::string v) {
        auto item = make_item(vb, k, v);
        item.setCas(1);
        item.setBySeqno(1);
...
TEST_F(CollectionsExpiryLimitTest, gat) {
    auto func = [this](Vbid vb, DocKey k, std::string v) {
