...
TEST_P(CollectionsManifestUpdate, update_scope_name_forced) {
    // Create diverged manifests
    CollectionsManifest cm, cm1;
    cm.add(ScopeEntry::Entry{"name1", 22});
    cm1.add(ScopeEntry::Entry{"name2", 22});
    setCollections(cookie, cm);
...
TEST_P(CollectionsManifestUpdate, force_update_multiple_changes) {
    ...
    // shop1 is open for business and fully stocked with an apple... forget it
    cm1.setForce(true);
    setCollections(cookie, cm1);
...
TEST(SharedMetaDataTable, basic) {
    Collections::SharedMetaDataTable<CollectionID, Meta, MetaView> table;
    ...
    Meta m3{"unknown", 10};
    EXPECT_THROW(table.dereference(8, MetaView{m3}), std::invalid_argument);

    // Release one and tell the table
    table.dereference(8, MetaView{m1});
    EXPECT_EQ(1, table.count(8));
    table.dereference(8, MetaView{m2});
    EXPECT_EQ(1, table.count(8));
    table.dereference(8, MetaView{m2});
    EXPECT_EQ(0, table.count(8));

    EXPECT_THROW(table.dereference(8, MetaView{m2}), std::invalid_argument);
}