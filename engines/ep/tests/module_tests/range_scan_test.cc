...
void RangeScanTest::testRangeScan(
    ...
    EXPECT_EQ(cb::engine_errc::no_such_key,
              store->continueRangeScan(*cookie,
...
TEST_P(RangeScanTest, continue_must_be_serialised) {
    ...
    auto continueParams =
            cb::rangescan::ContinueParameters{vbid,
                                              uuid,
                                              0,
                                              std::chrono::milliseconds(0),
                                              0,
...
TEST_P(RangeScanTest, random_sample_less_keys_than_samples) {
    ...
    // Now run the IO task to free the KVStore objects
    runNextTask(*task_executor->getLpTaskQ()[AUXIO_TASK_IDX],
                "RangeScanContinueTask");
}
...
TEST_P(RangeScanTest, cancel_when_yielding) {
    ...
    // This was scheduled by the cancelRangeScan request
    runNextTask(*task_executor->getLpTaskQ()[AUXIO_TASK_IDX],
                "RangeScanContinueTask");
}
...
#warning "There's no callback anymore, hmm"
