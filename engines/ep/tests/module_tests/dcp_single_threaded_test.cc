...
void STDcpConsumerTest::testConsumerNegotiates(
    ...
    MockDcpMessageProducers producers;
    using State = DcpConsumer::BlockingDcpControlNegotiation2::State;
    ...
    } while (consumer->public_getCurrentControlNegotiation().key !=
             testControl);

    // Check pre-negotiation state
    {
        auto& control = consumer->public_getCurrentControlNegotiation();
        ASSERT_EQ(State::PendingRequest, control.state);
        ASSERT_EQ(0, control.opaque);
    }
...

    {
        ...
        auto& control = consumer->public_getCurrentControlNegotiation();
        EXPECT_EQ(State::PendingResponse, control.state);
        EXPECT_GT(control.opaque, 0);
        EXPECT_EQ(control.opaque, producers.last_opaque);
    }
...
TEST_P(STDcpConsumerTest, ConsumerNegotiatesNoopMillisecondsProducerAccepts) {
    ...
    for (const auto& control : consumer->public_getPendingControls()) {
    ...
    testConsumerNegotiates(
            ...
            [](const MockDcpConsumer& consumer) {
                ...
                for (const auto& control :
                     consumer.public_getPendingControls()) {
                    if (control.key == DcpControlKeys::NoopInterval &&
                        control.value == "1") {
                        return false;
                    }
