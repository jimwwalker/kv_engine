...
protected:
    ...
    void createNode(Node node, vbucket_state_t vbState) {
        ...
        config += "dbname=" + std::string(test_dbname) + "-node_" +
                  std::to_string(node);
        extraEngines[node - 1] = SynchronousEPEngine::build(config);
        engines[node] = extraEngines[node - 1].get();
        // engines[node]->setName("Node" + std::to_string(node));
    ...
    DcpRoute createDcpRoute(
        ...
        return {vbid,
                ...
                setupProducer(producerNode, consumerNode, producerExpiryOutput),
                setupConsumer(producerNode, consumerNode)};
    }
    ...
    std::shared_ptr<MockDcpProducer> setupProducer(
            Node producer,
            Node consumer,
            EnableExpiryOutput enableExpiryOutput = EnableExpiryOutput::Yes,
            SyncReplication syncReplication = SyncReplication::Yes) {
        EXPECT_TRUE(engines[producer]) << "No engine for node:" << producer;

        int flags = cb::mcbp::request::DcpOpenPayload::IncludeXattrs |
                    cb::mcbp::request::DcpOpenPayload::IncludeDeleteTimes;
        auto mockProducer = std::make_shared<MockDcpProducer>(
                *engines[producer],
                create_mock_cookie(),
                "Node" + std::to_string(producer) + " to Node" +
                        std::to_string(consumer),
                flags,
                false /*startTask*/);

        // Create the task object, but don't schedule
        mockProducer->createCheckpointProcessorTask();

        // Need to enable NOOP for XATTRS (and collections).
        mockProducer->setNoopEnabled(true);

        mockProducer->scheduleCheckpointProcessorTask();
        if (enableExpiryOutput == EnableExpiryOutput::Yes) {
            mockProducer->setDCPExpiry(true);
        }

        mockProducer->setSyncReplication(syncReplication ==
                                         SyncReplication::Yes);

        return mockProducer;
    }

    std::shared_ptr<MockDcpConsumer> setupConsumer(Node producer,
                                                   Node consumer) {
        EXPECT_TRUE(engines[consumer]) << "No engine for node:" << consumer;

        // Setup the consumer.
        auto mockConsumer = std::make_shared<MockDcpConsumer>(
                *engines[consumer],
                create_mock_cookie(),
                "Node" + std::to_string(consumer) + " from Node" +
    ...
    void flushNodeIfPersistent(Node node = Node0) {
        ASSERT_TRUE(engines[node]) << "No engine for node:" << node;
    ...
    // All of the engines we can operate with, index 0 is always valid and is
    // the KVBucketTest::engine, and generally the active node in reflection
    // tests.
    // The indexing should be thought of as just 0 = node0, 1 = node1 etc...
    // as we may switch node1 from replica to active in some test scenarios
    std::array<SynchronousEPEngine*, 4> engines;

    // Owned pointers to the other engines, created on demand by tests
    std::array<SynchronousEPEngineUniquePtr, 3> extraEngines;
};
...
TEST_F(DCPLoopbackStreamTest, MissingKeyTrick) {
