...
void DCPLoopbackTestHelper::DcpRoute::transferCachedValue(
    ...
    EXPECT_EQ(expectedStatus, streams.second->messageReceived(std::move(msg)));
}
...
TEST_P(DCPCacheTransfer, CacheTransferOOMCancels) {
    ...
    // And manually drive the next message as we need the opaque so we can push
    // back to the producer a response
    ...
    if (fullEviction()) {
        ...
        // The CTS ended and switches to an ActiveStream
