...
void DCPLoopbackStreamTest::testBackfillAndInMemoryDuplicatePrepares(
    ...
    // the previous snap
    EXPECT_EQ(3, replicaVB->getCurrentSnapshotEnd(true));
    EXPECT_EQ(3, replicaVB->getCurrentSnapshotEnd(false));
    ...
    if (completeFinalSnapshot) {
        ...
        EXPECT_EQ(6, replicaVB->getCurrentSnapshotEnd(true));
        EXPECT_EQ(6, replicaVB->getCurrentSnapshotEnd(false));
...
TEST_F(DCPLoopbackStreamTest, MB_36948_SnapshotEndsOnPrepare) {
    ...
    ASSERT_TRUE(replicaVB);
    EXPECT_EQ(3, replicaVB->getCurrentSnapshotEnd(true));
    EXPECT_EQ(2, replicaVB->getCurrentSnapshotEnd(false));
...
void DCPLoopbackSnapshots::testSnapshots(int flushRatio) {
    ...
    auto expects = [&activeVB, &replicaVB](uint64_t seqno,
        ...
        EXPECT_EQ(activeSnapEnd, activeVB->getHighSeqno(true));
        EXPECT_EQ(activeSnapEnd, activeVB->getHighSeqno(false));
        EXPECT_EQ(activeSnapEnd, activeVB->getCurrentSnapshotEnd(true));
        EXPECT_EQ(activeSnapEnd, activeVB->getCurrentSnapshotEnd(false));

        EXPECT_EQ(seqno, replicaVB->getHighSeqno(true));
        EXPECT_EQ(seqno, replicaVB->getHighSeqno(false));
        EXPECT_EQ(replicaSnapEnd, replicaVB->getCurrentSnapshotEnd(true));
        EXPECT_EQ(replicaSnapEnd, replicaVB->getCurrentSnapshotEnd(false));
    };
