...
TEST_F(WarmupTest, MB_32577) {
    ...
    // request using DCP
    auto& readerQueue = *task_executor->getLpTaskQ(TaskType::Reader);
    while (store->isPrimaryWarmupLoadingData()) {
...
TEST_F(WarmupOrderTest, pauseResumeVisit) {
    ...
    for (auto ii = gsl::narrow_cast<Vbid::id_type>(store->getVBMapSize() - 1);
         ii >= 0;
         ii -= 3) {
...
TEST_P(MB_34718_WarmupTest, getTest) {
    ...
    store_item(vbid, key, "meh", makeExpiryTime(3600));
    flush_vbucket_to_disk(vbid);
...
TEST_F(WarmupDiskTest, noDataFileCollectionCountsTest) {
    ...
    while (getKVBucket()->isPrimaryWarmupLoadingData()) {
...
TEST_F(WarmupDiskTest, readOnlyDataFileSetVbucketStateTest) {
    ...
    EXPECT_EQ(WarmupState::State::Done,
              getKVBucket()->getPrimaryWarmup()->getWarmupState());
    EXPECT_FALSE(getKVBucket()->isPrimaryWarmupLoadingData());
...
TEST_F(WarmupTest, CrashWarmupAfterInitialize) {
...

    {
        ...
        do {
            ...
            executor.runCurrentTask();
            executor.completeCurrentTask();
        } while (kvBucket->isPrimaryWarmupLoadingData() &&
...
protected:
    ...
    void testWarmupAbortedOnDiskError(WarmupState::State injectState,
        ...
        auto kvBucket = engine->getKVBucket();
        auto warmup = kvBucket->getPrimaryWarmup();
        EXPECT_TRUE(kvBucket->isPrimaryWarmupLoadingData());
...
TEST_F(WarmupTest, WarmupZeroThreshold) {
    ...
    EXPECT_FALSE(store->isPrimaryWarmupLoadingData());
