...
//     void TearDown() {
//         /* Like in a vbucket we want the list to be erased before HashTable
//         is
//            is destroyed. */
//         basicLL.reset();
//     }
...
//     /**
//      * Adds 'numItems' number of new items to the linked list, from
//      startSeqno.
...
//             StoredDocKey key = makeStoredDocKey(keyPrefix +
//             std::to_string(i)); Item item(key,
//                       0,
//                       0,
//                       val.data(),
//                       val.length(),
...
//         basicLL->updateHighSeqno(listWriteLg,
//         *(newSv->toOrderedStoredValue()));
//     }
...
//     /* We need a HashTable because StoredValue is created only in the
//     HashTable
...
//     addNewItemsToList(
//             numItems + /* +1 is update, another +1 for next */ 2, keyPrefix,
//             1);
...
//     auto ownedSv = releaseFromHashTable(keyPrefix +
//     std::to_string(numItems)); OrderedStoredValue* nonOwnedSvPtr =
//     ownedSv->toOrderedStoredValue(); size_t svSize = ownedSv->size(); size_t
//     svMetaDataSize = ownedSv->metaDataSize();
...
//     auto itr = getRangeIterator();
//     /* Since there are no items in the list to iterate over, we expect itr
//     start
//        to be end */
...
//     */ seqno_t exp[] = {1, 3, 4}; expectedSeqno =
...
//     /* MB-24376: rangeRead has to stop if it encounters an OSV with a seqno
//     of
...
//        the purger, hence we do not expect the update to create a stale copy
//        of the updated item */
...
//     /* Add an item which doesn't yet have a seqno. Such a scenario is
//     possible
...
//     addNewItemsToList(
//             1 + 1 /* one stale item */ + numItems / 2, keyPrefix, numItems /
//             2);
