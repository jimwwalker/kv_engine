...
TEST_F(WarmupTest, produce_delete_times) {
    ...
    auto producer = createDcpProducer(cookie, {}, false, true /*deleteTimes*/);
    ...
    // Now a new delete, in-memory won't have a delete time (it will have 0)
    storeAndDeleteItem(
    ...
    EXPECT_EQ(0, dcp_last_delete_time);
    EXPECT_EQ(PROTOCOL_BINARY_CMD_DCP_DELETION, dcp_last_op);
