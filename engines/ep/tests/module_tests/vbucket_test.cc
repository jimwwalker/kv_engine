...
TEST_P(VBucketTest, AddExpiry) {
    ...
    ASSERT_EQ(AddStatus::Success, addOne(k, makeExpiryTime(5)));
    EXPECT_EQ(AddStatus::Exists, addOne(k, makeExpiryTime(5)));

    const auto* v = this->vbucket->ht.findForRead(k).storedValue;
    EXPECT_TRUE(v);
    EXPECT_FALSE(v->isExpired(gsl::narrow<uint32_t>(ep_real_time())));
    EXPECT_TRUE(v->isExpired(makeExpiryTime(6)));

    TimeTraveller biffTannen(6);
    EXPECT_TRUE(v->isExpired(ep_real_time()));

    EXPECT_EQ(AddStatus::UnDel, addOne(k, makeExpiryTime(5)));
    EXPECT_TRUE(v);
    EXPECT_FALSE(v->isExpired(gsl::narrow<uint32_t>(ep_real_time())));
    EXPECT_TRUE(v->isExpired(makeExpiryTime(6)));
}
