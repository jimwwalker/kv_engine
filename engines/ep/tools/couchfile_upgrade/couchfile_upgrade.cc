...
static void usage() {
    std::cout <<
            R"(Usage:
              -x   Optional: Enable XATTR splitting. Any documents in the input
                   file with extended attributes will result in multiple
                   documents in the output file. The extended attributes will be
                   placed in their own name-spaced documents.
              -v   Optional: Run with verbose output to stdout.
              -i   Required: Input filename.
              -o   Required: Output filename to be created. Note -x is specified
                   then an <output>.xattrs file will temporarily exist during
                   processing.)"
              << std::endl;
}
...
static ProgramOptions parseArguments(int argc, char** argv) {
    ...
    while ((cmd = getopt(argc, argv, "i:o:vx")) != -1) {
        switch (cmd) {
        case 'v': {
        ...
        case 'x': {
            pOptions.options.set(Options::SplitXattrs);
            std::cout << "Splitting XATTRs\n";
            break;
        }
        case 'i': {
...
int main(int argc, char** argv) {
    ...
    if (!success) {
        std::cerr << "Terminating with exit(1)\n";
    }
