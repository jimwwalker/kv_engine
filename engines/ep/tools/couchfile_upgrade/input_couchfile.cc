...
static int upgradeCallback(Db* db, DocInfo* docinfo, void* ctx) {
    ...
    if (errcode) {
        ...
        if (mcbp::datatype::is_xattr(documentMetaData->getDataType()) &&
            context->outputXattrs) {
            Doc document = *doc.getDoc();
            // Direct the xattrs and document to a different file
            document = context->outputXattrs->processXattrs(&document, docinfo);
            context->outputXattrs->processDocument(&document, docinfo, false);
        } else {
            context->output.processDocument(doc.getDoc(), docinfo, true);
        }
    }
    return 0;
}
...
void InputCouchFile::upgrade(OutputCouchFile& output) const {
    std::unique_ptr<XattrCouchFile> xattrs;
    if (options.test(Options::SplitXattrs)) {
        xattrs = std::make_unique<XattrCouchFile>(
                options, output.getFilename() + ".xattrs");
    }
    UpgradeCouchFileContext context{output, xattrs.get()};
    ...
    bool removeFailoverTable = false;

    if (xattrs && xattrs->getNumberProcessed() > 0) {
        xattrs->commit();
        output.merge(*xattrs);
        if (remove(xattrs->getFilename().c_str()) != 0) {
            throw std::runtime_error("InputCouchFile::upgrade remove errno:" +
                                     std::to_string(errno));
        }
        output.commit();
        removeFailoverTable = true;
    }

    // Finally read, adjust and write _local/vbstate
    auto vbstate = getLocalDocument("_local/vbstate");
    output.setVBState(vbstate, removeFailoverTable);
    output.commit();
}
