...
std::chrono::seconds Snapshot::getAge() const {
    return std::chrono::duration_cast<std::chrono::seconds>(
            std::chrono::steady_clock::now() - lastTouched);
}
...
cb::engine_errc Cache::getOrPrepare(
    ...
    auto rv = Snapshots::prepare(kvs, path, vbid);
    ...
    auto [emplaceItr, emplaced] = locked->emplace(
            vbid, std::make_shared<Snapshot>(std::move(manifest)));
...
cb::engine_errc Cache::releaseSnapshot(CookieIface& cookie,
    ...
    return Snapshots::release(path, entry->manifest["uuid"].get<std::string>());
}

cb::engine_errc Cache::releaseSnapshot(CookieIface& cookie,
                                       std::string_view path,
                                       Vbid vbid) {
    auto entry = find(vbid, [](Map::iterator i, Map& m) {
        std::shared_ptr<Snapshot> rv;
        rv.swap(i->second);
        m.erase(i);
        return rv;
    ...
    if (!entry) {
        EP_LOG_WARN("Snapshots::releaseSnapshot no such snapshot with {}",
                    vbid);
        return cb::engine_errc::no_such_key;
    }
...
cb::engine_errc Cache::getFileInfo(
        CookieIface& cookie,
        std::string_view uuid,
        std::size_t file_id,
        const std::function<void(const nlohmann::json&)>& callback) {
    ...
    if (entry) {
        // Find the id
        entry->manifest.dump();
        auto& files = entry->manifest["files"];
...
void Cache::addSnapshot(Vbid vbid, nlohmann::json manifest) {
    auto snap = std::make_shared<Snapshot>(manifest);
    auto locked = map.lock();
    auto [emplaceItr, emplaced] = locked->emplace(vbid, std::move(snap));

    // One snapshot per vbucket
    Expects(emplaced);
}
...
int Cache::purge(std::string_view path, std::chrono::seconds maxAge) {
    ...
    // Move all candidates to purge into a local container, this local
    // container's destruction will free the memory side of things
    std::vector<MapValue> toRelease = map.withLock([now, maxAge](auto& map) {
        std::vector<MapValue> toRelease;
        for (auto itr = map.begin(); itr != map.end();) {
            if ((itr->second->lastTouched + maxAge) <= now) {
                toRelease.emplace_back(std::move(itr->second));
                itr = map.erase(itr);
            } else {
                ++itr;
            }
    ...
    // Purge all candidates from disk
    size_t success{0};
    for (const auto& snapshot : toRelease) {
        if (Snapshots::release(path,
                               snapshot->manifest["uuid"].get<std::string>()) ==
            cb::engine_errc::success) {
            ++success;
        } else {
            EP_LOG_WARN("Cache::purge failed to release uuid:{} from {}",
                        snapshot->manifest["uuid"].get<std::string>(),
                        path);
        }
    }
...
Snapshots::Cache::MapValue Cache::find(
        Vbid vbid, std::function<MapValue(Map::iterator, Map&)> apply) {
    return map.withLock([vbid, apply](auto& map) {
...
std::ostream& operator<<(std::ostream& os, const Snapshot& s) {
    return os << s.lastTouched.time_since_epoch().count() << " " << s.manifest;
}
