...
void CouchKVStore::addStats(const AddStatFn& add_stat, CookieIface& c) const {
...
CouchKVStore::prepareSnapshotImpl(
        const std::filesystem::path& snapshotDirectory, Vbid vb) {
    DbHolder db(*this);
    couchstore_error_t err = openDB(vb, db, COUCHSTORE_OPEN_FLAG_RDONLY);
    if (err != COUCHSTORE_SUCCESS) {
        logOpenError(__func__,
...
static int scanCallback(Db* db, DocInfo* docinfo, void* ctx) {
    ...
    auto rv = makeItemFromDocInfo(vbucketId,
                                  *docinfo,
                                  *metadata,
                                  value,
                                  fetchCompressed,
