...
std::pair<cb::engine_errc, RangeScanId> EPBucket::createRangeScan(
        Vbid vbid,
        const DocKey& start,
        const DocKey& end,
    ...
    return vb->createRangeScan(
            start, end, handler, cookie, keyOnly, snapshotReqs, samplingConfig);
}
...
cb::engine_errc EPBucket::cancelRangeScan(Vbid vbid, RangeScanId uuid) {
    auto vb = getVBucket(vbid);
    if (!vb) {
        ++stats.numNotMyVBuckets;
        return cb::engine_errc::not_my_vbucket;
