...
public:
    ...
    /**
     * Return the end seqno for the current snapshot. This logically matches
     * the end which would be returned by getSnapshotInfo, but exists because
     * in the case the caller wants the visible max seqno
     * (supportsSyncWrites:false) a snapshot_range_t *could* be generated with
     * start > end, which is an exception.
     *
     * @param supportsSyncWrites does the caller support sync writes, if false
     *        then the returned value is the maximum visible seqno for the
     *        snapshot.
     ...
     *          complete snapshot.
     */
    uint64_t getSnapshotEndSeqno(bool supportsSyncWrites) const;
