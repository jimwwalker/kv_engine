...
void Flush::updateStats(const DocKey& key,
    ...
    // Of interest next is the state of old vs new. This update can change to
    // be insert/remove or update
    //
    // The old key can be live, deleted or dropped.
    // The new key can be live or deleted.
    //
    // new key is live:
    //   * old key dropped: Key is an insert => items += 1, diskSiz += size
    ...
    //
    // For the insert/remove cases, if the item is not committed, item count
    // doesn't change (that is done inside
    // Flush::StatisticsUpdate:: insert/remove)
