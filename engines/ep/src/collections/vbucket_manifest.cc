...
bool Manifest::update(::VBucket& vb, const Collections::Manifest& manifest) {
    ...
    if (!rv.is_initialized()) {
        ...
        std::vector<Addition>& additions = rv->first;
        std::vector<CollectionID>& deletions = rv->second;

        auto finalDeletion = applyDeletions(vb, deletions);
        ...
        auto finalAddition = applyCreates(vb, additions);
...
Manifest::processResult Manifest::processManifest(
    ...
    std::vector<CollectionID> deletions;
    ...
    for (auto scopeItr = manifest.beginScopes();
        ...
        for (const auto& m : scopeItr->second.collections) {
            ...
            if (mapItr == map.end()) {
                additions.push_back({std::make_pair(scopeItr->first, m.id),
