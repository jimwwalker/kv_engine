...
Warmup::Warmup(Warmup& warmup,
    ...
    : Warmup(warmup.store,
             warmup.config,
             std::move(warmupDoneFunction),
             memoryThreshold,
             itemsThreshold,
             std::move(name)) {
    estimatedWarmupCount = warmup.estimatedWarmupCount.load();
...
Warmup::WarmupAccessLogState Warmup::doWarmup(
    ...
    const auto chunkDuration = std::chrono::milliseconds{
            config.getWarmupBackfillScanChunkDuration()};
    const auto batchSize = config.getWarmupBatchSize();

    // Keep loading batches until time is up.
    while (harvester.loadAndApply(
            batchSize,
            &cookie,
            &batchWarmupCallback,
