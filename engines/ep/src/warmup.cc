...
public:
    LoadStorageKVPairCallback(
            ...
            boost::optional<const std::chrono::steady_clock::duration>
                    deltaDeadlineFromNow = boost::none);
...
bool WarmupVbucketVisitor::visit(VBucket& vb) {
    ...
    if (!currentScanCtx) {
        ...
        auto kvLookup = std::make_unique<LoadStorageKVPairCallback>(
                ...
                boost::optional<const std::chrono::steady_clock::duration>{
                        chunkDuration});
        currentScanCtx = kvstore->initScanContext(
                std::move(kvLookup),
    ...
    if (errorCode == scan_again) {
        ...
        if (!needToScanAgain) {
            ...
            kvstore->destroyScanContext(currentScanCtx);
            currentScanCtx = nullptr;
        }
...
void LoadStorageKVPairCallback::callback(GetValue& val) {
    ...
    auto& visitWarmupHook = this->epstore.getEPEngine().visitWarmupHook;
    if (visitWarmupHook) {
        visitWarmupHook();
    }
...
void Warmup::stop() {
    ...
    processCreateVBucketsComplete(ENGINE_DISCONNECT);
}
...
void Warmup::processCreateVBucketsComplete(ENGINE_ERROR_CODE status) {
...
void Warmup::populateVBucketMap(uint16_t shardId) {
    ...
    if (++threadtask_count == store.vbMap.getNumShards()) {
        ...
        processCreateVBucketsComplete(ENGINE_SUCCESS);
