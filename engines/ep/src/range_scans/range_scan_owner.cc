...
=======
            ...
            if (map.size() == 1) {
                // This vbucket now has one scan, inform the timeout task so it
                // can force cancel this scan if it exceeds the maxScanDuration.
                // If this scan does complete/cancel before the deadline, the
                // task will still run and evaluate what it needs todo - sleep
                // forever or sleep until another scan may require cancelling.
                timeoutTask->updateNextExpiryTime(
                        std::chrono::steady_clock::now() +
                        readyScans->getMaxDuration());
>>>>>>> 753b07ba8 (MB-35297: Make RangeScan config duration and buffering dynamic)
            }
