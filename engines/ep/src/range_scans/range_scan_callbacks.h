...
public:
    ...
    /**
     * Key Received for a Key only scan
     */
    virtual void handleKey(DocKey key) = 0;

    /**
     * Item Received for a Key/Value scan
     */
...
/**
 * callback class which is invoked second by the scan if no value is available
 * in the cache. The full Item is passed to the callback.
 */
