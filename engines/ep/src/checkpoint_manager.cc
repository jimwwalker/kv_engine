...
CheckpointManager::ItemsForCursor CheckpointManager::getItemsForCursor(
    ...
    ItemsForCursor result(
            ...
            (*cursor.currentCheckpoint)->getVisibleSnapEndSeqno());
    ...
    while ((result.moreAvailable = incrCursor(cursor))) {
        if (enteredNewCp) {
            result.checkpointType =
                    (*cursor.currentCheckpoint)->getCheckpointType();
            // maxVisibleSeqno must be from the final checkpoint we include.
            result.maxVisibleSeqno =
                    (*cursor.currentCheckpoint)->getVisibleSnapEndSeqno();
...
snapshot_info_t CheckpointManager::getSnapshotInfo(bool supportsSyncWrites) {
    LockHolder lh(queueLock);

    auto seqno = supportsSyncWrites ? lastBySeqno : maxVisibleSeqno;
    ...
    snapshot_info_t info(
            ...
            {openCkpt.getSnapshotStartSeqno(),
             supportsSyncWrites ? openCkpt.getSnapshotEndSeqno()
                                : openCkpt.getSnapshotVisibleEndSeqno()});
    ...
    // no items in it.
    if (openCkpt.getNumItems() == 0 &&
        static_cast<uint64_t>(seqno) < info.range.getStart()) {
        info.range = snapshot_range_t(seqno, seqno);
    }
