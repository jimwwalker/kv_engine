...
uint64_t CheckpointManager::getSnapshotEndSeqno(bool supportsSyncWrites) const {
    // Follow what getSnapshotInfo does, but only for end-seqno
    LockHolder lh(queueLock);
    const auto& openCkpt = getOpenCheckpoint_UNLOCKED(lh);

    if (openCkpt.getNumItems() == 0 &&
        static_cast<uint64_t>(lastBySeqno) < openCkpt.getSnapshotStartSeqno()) {
        return supportsSyncWrites ? lastBySeqno : maxVisibleSeqno;
    }

    return supportsSyncWrites ? openCkpt.getSnapshotEndSeqno()
                              : openCkpt.getVisibleSnapshotEndSeqno();
}
