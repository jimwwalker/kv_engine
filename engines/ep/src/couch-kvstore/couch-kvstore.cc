...
static constexpr const char* scopesName = "_local/collections/scopes";
...
Collections::KVStore::Manifest CouchKVStore::getCollectionsManifest(Vbid vbid) {
    ...
    // @todo: change this to a presence check, no need to read the entire
    // document, convert and create a vector. patch 3/5 will manage the
    // life-time of this document.
    rv.droppedCollectionsExist = !getDroppedCollections(*db.getDb()).empty();
    return rv;
}
    ...
    if (collections.getLocalDoc()) {
        ...
        for (const auto& entry : *fbData->entries()) {
            ...
            if (result == collectionsMeta.droppedCollections.end()) {
                ...
                // Found in the dropped list - transfer start-seqno
