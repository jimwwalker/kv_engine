...
void CouchKVStore::saveCollectionStats(Db& db,
    ...
    writeLocalDoc(docName, stats.getLebEncodedStats());
...
void CouchKVStore::writeLocalDoc(std::string_view name, std::string_view data) {
    pendingLocalReqsQ.emplace_back(name, data);
}

void CouchKVStore::deleteLocalDoc(std::string_view name) {
    pendingLocalReqsQ.emplace_back(name, CouchLocalDocRequest::IsDeleted{});
}
...
void CouchKVStore::updateScopes(Db& db) {
    ...
    auto buf = encodeScopes(collectionsMeta,
                            scopes.getLocalDoc() ? scopes.getBuffer() : empty);
    writeLocalDoc(Collections::scopesName,
                  {reinterpret_cast<const char*>(buf.data()), buf.size()});
}
...
couchstore_error_t CouchKVStore::updateLocalDocuments(Db& db) {
    ...
    // Build a vector of localdoc pointers from the pending updates
    std::vector<LocalDoc*> localDocumentsPtrs;
    localDocumentsPtrs.reserve(pendingLocalReqsQ.size());

    for (auto& lDoc : pendingLocalReqsQ) {
        localDocumentsPtrs.push_back(lDoc.getLocalDoc());
    }
...
CouchLocalDocRequest::CouchLocalDocRequest(std::string_view key,
                                           std::string_view value)
    : key(key), value(value) {
    doc.id.buf = const_cast<char*>(this->key.data());
    doc.id.size = this->key.size();
    doc.json.buf = const_cast<char*>(this->value.data());
    doc.json.size = this->value.size();
}

CouchLocalDocRequest::CouchLocalDocRequest(std::string_view key,
                                           CouchLocalDocRequest::IsDeleted)
    : CouchLocalDocRequest(key, std::string_view{}) {
    doc.deleted = 1;
}
