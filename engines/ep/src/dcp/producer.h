...
public:
    ...
    public:
        ...
        */
         ...
         * will have stored the value of bytesOutstanding and the time iff
         * bytesOutstanding is different from the recorded value. This allows
         * the isStuck function to determine if the bytesOutstanding has not
         * changed for the given limit.
         *
         * @param limit The limit to check against.
         * @return true if the bytesOutstanding has not changed within the given
    ...
    private:
        ...
        /// When paused, update this value to the current number of bytes
        /// outstanding.
        std::optional<size_t> lastCheckedBytesOutstanding;
        /// When paused, record the time when lastCheckedBytesOutstanding
        /// changed value.
...
protected:
     ...
     * configured from the producer's name, so we can opt producer's into this
     * protection.
     */
    bool shouldDisconnectWhenStuck{false};
    std::chrono::seconds stuckTimeout{std::chrono::seconds(10)};
};
