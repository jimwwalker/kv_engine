...
bool SnapshotMarker::isEqual(const DcpResponse& rsp) const {
    ...
    auto d = [](const SnapshotMarker& z) {
        std::cerr << "z.vbucket_:" << z.vbucket_ << std::endl;
        std::cerr << "z.start_seqno_:" << z.start_seqno_ << std::endl;
        std::cerr << " z.end_seqno_:" << z.end_seqno_ << std::endl;
    };
...
bool MutationConsumerMessage::isEqual(const DcpResponse& rsp) const {
    ...
    bool eq = emd && other.emd;
    if (emd && other.emd) {
        auto ext1 = emd->getExtMeta();
        auto ext2 = other.emd->getExtMeta();
        eq |= ext1.second == ext2.second; // length check
        if (eq) {
            eq = std::memcmp(ext1.first, ext2.first, ext1.second) == 0;
        }
    }
    return eq && MutationResponse::isEqual(rsp);
}
