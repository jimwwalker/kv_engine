    ...
    if (!outstandingItemsResult.items.empty()) {
        ...
        if (!mutations.empty()) {
            ...
            if (!firstMarkerSent && lastReadSeqno < snap_end_seqno_) {
                ...
                // needed.
                firstMarkerSent = true;
                snapshotSeqnoAdvanced(
                        snap_start_seqno_, snap_end_seqno_, MARKER_FLAG_MEMORY);
                ...
                if (vb) {
                    if (vb->getState() == vbucket_state_replica) {
                         ...
                         * snapshot shot and we're a replica. Then our snapshot
                         ...
                         * need when no items for the collection we're streaming
                ...
                snapshotSeqnoAdvanced(highNonVisibleSeqno.value(),
                                      highNonVisibleSeqno.value(),
                                      MARKER_FLAG_MEMORY);
            } else {
                std::cerr << "hmm " << highNonVisibleSeqno.value_or(999) << " "
                          << curChkSeqno << "\n";
            }
        }
    }
...
void ActiveStream::snapshotSeqnoAdvanced(uint64_t start,
                                         uint64_t end,
                                         uint32_t flags) {
    if (!firstMarkerSent) {
        start = std::min(snap_start_seqno_, start);
        firstMarkerSent = true;
    }
